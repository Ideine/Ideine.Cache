...
{
	Task PutOnCache<T>(T item, string key, int page, DateTimeOffset? absoluteExpiration = default);
	Task PutOnCache<T>(T item, string key, DateTimeOffset? absoluteExpiration = default);
	...
	Task<T> GetOrFetch<T>(string key, Func<Task<T>> fetchFunc, DateTimeOffset? absoluteExpiration = default);
	...
	/// <param name="absoluteExpiration">An optional expiration date.</param>
	/// <typeparam name="T">The 1st type parameter.</typeparam>
	Task<T> GetOrFetch<T>(string key, int page, Func<Task<T>> fetchFunc, DateTimeOffset? absoluteExpiration = default);
